...
class ArithmeticsAdditionFixedDigitPreprocessConfig(BaseModel):
    model_config = ConfigDict(extra="forbid")

    tokenizer: TokenizerName = Field(default="byteoss", description="Tokenizer: gpt2|byteoss.")
    encoding: str = Field(
        default="gpt2", description="tiktoken encoding name (ignored using default byteoss tokenizer)."
    )

    digit_width: PositiveInt = Field(default=4, description="Number of digits for each addend (x).")
    ...
    output_dir: Path | None = Field(
        default=None,
        description=(
            "Output directory (default: NANOGPT_DATA_ROOT/arithmetics-addition-fixed_digit "
            "or data/arithmetics-addition-fixed_digit)."
        ),
    )
...
def _reverse_digits(value: int, *, width: int) -> str:
    s = f"{value:0{width}d}"
    return s[::-1]


def _sample_reversed_digits(*, rng: np.random.Generator, digit_width: int) -> str:
    """
    Sample a uniform fixed-width decimal integer in [0, 10**digit_width) and return its digits
    in reversed order (length=digit_width).

    This samples digits directly to avoid NumPy int64 bounds for large widths (e.g. digit_width=32).
    """
    if digit_width < 1:
        raise ValueError(f"digit_width must be >= 1, got {digit_width}")
    digits = rng.integers(0, 10, size=(digit_width,), dtype=np.uint8)
    return (digits + _ASCII_ZERO).tobytes().decode("ascii")


def _add_reversed_fixed_width(a_rev: str, b_rev: str, *, out_width: int) -> str:
    """Add fixed-width reversed decimal digit strings and return a fixed-width reversed sum."""
    if not a_rev:
    ...
    if carry:
        raise RuntimeError("carry overflowed fixed out_width; increase out_width")
    return "".join(out_chars)


def _make_example(*, rng: np.random.Generator, digit_width: int) -> tuple[str, str]:
    a_rev = _sample_reversed_digits(rng=rng, digit_width=digit_width)
    b_rev = _sample_reversed_digits(rng=rng, digit_width=digit_width)
    c_rev = _add_reversed_fixed_width(a_rev, b_rev, out_width=digit_width + 1)
    ...
    try:
        ...
        for i in range(num_examples):
            ...
            if tokenizer == "byteoss":
                ids = _encode_example_byteoss(context=context, text=text, bos_id=bos_id, eos_id=eos_id)
                if len(ids) != seq_len:
                    raise RuntimeError(
                        f"Unexpected token_seq_len={len(ids)} (expected {seq_len}) for example: {context + text!r}"
                    )
                actual_len = seq_len
...
def _parse_config(argv: Sequence[str] | None = None) -> ArithmeticsAdditionFixedDigitPreprocessConfig:
    parser = argparse.ArgumentParser(
        description="Arithmetics preprocessing (fixed-width addition, tokenizer switchable)"
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
    ...
    parser.add_argument("--digit_width", type=int, default=4, help="Addend digit width x (default: 4)")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output dir (default: NANOGPT_DATA_ROOT/arithmetics-addition-fixed_digit "
            "or data/arithmetics-addition-fixed_digit)"
        ),
    )
    ...
    try:
        return ArithmeticsAdditionFixedDigitPreprocessConfig.model_validate(vars(args))
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    # Upper-bound seq_len: token count is always <= bytes length (BPE merges only reduce),
    # plus BOS/EOS.
    example_context, example_text = _make_example(rng=np.random.default_rng(0), digit_width=digit_width)
    example_bytes_len = len(_encode_ascii_bytes(f"{example_context}{example_text}"))
    seq_len = example_bytes_len + 2
    ...
    print(f"wrote arithmetics-addition-fixed_digit dataset to {output_dir}")
